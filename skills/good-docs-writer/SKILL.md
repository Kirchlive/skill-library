...
description: >
  ...
  Do NOT trigger for opinion essays, personal narratives, "year in review" posts,
  or non-technical writing — no Good Docs template fits those.
...
This skill is blog-flavored: it produces a draft with a hook and a voice, not neutral reference docs.

## Process

### 1. Classify the post intent

If the user used the explicit form (`$good-docs-writer walkthrough adding auth to Astro`), resolve the intent token via the "Explicit-form intent tokens" table in [references/template-index.md](references/template-index.md) and skip classification. Unknown token → fall back to the decision matrix.
...
Also read `references/writing-tips.md` once per session if you need general voice/concision guidance. Never read `references/style-guide.md` wholesale (~48 KB) — consult it only to verify a specific style call, and prefer `Grep` over `Read` on that file.
...
- **Topic is tied to the user's repo** (e.g. "blog post about how our auth flow works"): explore the repo with `Glob` / `Grep` / `Read` to collect concrete file paths, function names, code excerpts. A focused scan is enough — don't sweep the whole repo.
