...
## Explicit-form intent tokens

When the user invokes `$good-docs-writer <token> <topic>`, map the token directly:

| Token | Template key |
|-------|--------------|
| `explainer` | `concept` |
| `walkthrough` | `tutorial` |
| `recipe` / `how-to` | `how-to` |
| `quickstart` | `quickstart` |
| `debug-story` | `troubleshooting` |
| `release-post` | `release-notes` |
| `cheat-sheet` | `reference` |
| `glossary` | `glossary` |
| `install-guide` | `installation-guide` |
| `api-start` | `api-getting-started` |

Unknown token → fall back to the decision matrix above; do not guess a key.
