...
Template-agnostic, apply to every blog post:

- [`writing-tips.md`](./writing-tips.md) — Good Docs writing tips (concise, accurate, accessible). Read once per session.
- [`style-guide.md`](./style-guide.md) — Good Docs style guide (sentence case, voice, grammar). ~48 KB. **Do not read wholesale.** Consult only to verify a specific style call, and prefer `Grep` over `Read` on this file.
...
3. **`Read` `writing-tips.md`** once per session if the guidance is needed. The full `style-guide.md` is large (~48 KB) — never read it whole; `Grep` it only to verify a specific style call.
