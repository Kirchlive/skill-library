---
name: skill-reviewer
description: Reviews Model skill packages for readiness, triggers, safety boundaries, resources, and evidence. Invoke when users ask to audit, grade, or production-check an existing skill.
allowed-tools:
...
2. Call `review_skill_package`.
   - Use `profile="model"` unless the user explicitly asks for portability against another skill spec.
